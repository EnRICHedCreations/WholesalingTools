...
function generateAIPrompt(data) {
    try {
        const promptEl = document.getElementById('aiPromptOutput');
        const linkEl = document.getElementById('chatGPTLink');
        
        if (!promptEl || !linkEl) return;
...
Format the response in a clear, organized manner suitable for real estate investment analysis.`;
        ...
        linkEl.href = `https://chat.openai.com/?q=${encodedPrompt}`;
    } catch (error) {
